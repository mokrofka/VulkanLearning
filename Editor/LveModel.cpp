...
namespace std {
	template <>
	struct hash<LveModel::Vertex> {
		u64 operator()(LveModel::Vertex const& vertex) const {
			u64 seed = 0;
...
void LveModel::createVertexBuffers(const std::vector<Vertex>& vertices) {
	...
	stagingzBuffer.map();
	stagingzBuffer.writeToBuffer((void*)vertices.data());
	...
	lveDevice.copyBuffer(stagingzBuffer.getBuffer(), vertexBuffer->getBuffer(), bufferSize);
}
...
void LveModel::Builder::loadModel(const std::string& filepath) {
	...
	for (auto& shape : shapes) {
		for (auto& index : shape.mesh.indices) {
			...
			// it's for indecis but bad performance!
			/*
