...
std::vector<char> LvePipeline::readFile(const std::string& shaderName) {

	std::string path = "shaders/compiled/" + shaderName + ".spv";
...
void LvePipeline::createGraphicsPipeline(const std::string&        vertFilepath, const std::string& fragFilepath,
	...
	auto& attributDescriptions = configInfo.attributeDescriptions;

	VkPipelineVertexInputStateCreateInfo vertexInputInfo{};
	vertexInputInfo.sType                           = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
	vertexInputInfo.vertexAttributeDescriptionCount = (u32)attributDescriptions.size();
	...
	if (vkCreateGraphicsPipelines(device.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
		throw std::runtime_error("failed tocreate graphics pipeline");
	}
}
...
void LvePipeline::defaultPipelineConfigInfo(PipelineConfigInfo& configInfo) {
	...
	configInfo.dynamicStateInfo.dynamicStateCount = (u32)configInfo.dynamicStateEnables.size();
